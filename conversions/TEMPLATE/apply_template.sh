...
#
# ult3edit roster edit "$ROST" --slot 0 --in-party --food 500
...
# MONA=$(find_file "MONA")
# ult3edit bestiary edit "$MONA" --monster 0 --hp 60 --attack 35 --backup
...
#         ult3edit bestiary import "$MON" "$JSON" --backup
#     fi
# done
...
# CONG=$(find_file "CONG")
# ult3edit combat edit "$CONG" --tile 5 5 0x84 --backup
...
#         ult3edit combat import "$CON" "$JSON" --backup
#     fi
# done
...
# MAPA=$(find_file "MAPA")
# ult3edit map fill "$MAPA" --x1 0 --y1 0 --x2 63 --y2 63 --tile 0x04 --backup
# ult3edit map set "$MAPA" --x 30 --y 30 --tile 0x1C
...
#     if [ -f "$MAP" ] && [ -f "$SRC" ]; then
#         ult3edit map compile "$SRC" --output "${SCRIPT_DIR}/build/map${letter,,}.json"
#         ult3edit map import "$MAP" "${SCRIPT_DIR}/build/map${letter,,}.json" --backup
#     fi
# done
...
#     SRC="${SOURCES_DIR}/map${letter,,}.map"
#     if [ -f "$MAP" ] && [ -f "$SRC" ]; then
#         ult3edit map compile "$SRC" --dungeon --output "${SCRIPT_DIR}/build/map${letter,,}.json"
#         ult3edit map import "$MAP" "${SCRIPT_DIR}/build/map${letter,,}.json" --backup
#     fi
# done
...
#         ult3edit special import "$FILE" "$JSON" --backup
#     fi
# done
...
#     if [ -f "$TLK" ]; then
#         ult3edit tlk edit "$TLK" --find "EXODUS" --replace "YOUR VILLAIN" \
...
#     if [ -f "$TLK" ] && [ -f "$SRC" ]; then
#         ult3edit tlk build "$SRC" "$TLK" --backup
#     fi
# done
...
# ult3edit shapes compile "${SOURCES_DIR}/tiles.tiles" --format json \
#     --output "${SCRIPT_DIR}/build/tiles.json"
# ult3edit shapes import "$SHPS" "${SCRIPT_DIR}/build/tiles.json" --backup

# Option B: Edit individual glyphs
# ult3edit shapes edit "$SHPS" --glyph 0 --data "00 14 08 14 22 08 14 00" --backup
...
# ult3edit patch compile-names "${SOURCES_DIR}/names.names" --ult3 "$ULT3" \
#     --format hex > "${SCRIPT_DIR}/build/nametable.hex"
# ult3edit patch edit "$ULT3" --region name-table \
#     --data "$(cat "${SCRIPT_DIR}/build/nametable.hex")" --backup

# Relocate moongates (8 X + 8 Y coordinates, hex bytes)
# ult3edit patch edit "$ULT3" --region moongate-x --data "0A 14 1E 28 32 1E 14 0A"
# ult3edit patch edit "$ULT3" --region moongate-y --data "0A 0A 0A 1E 32 32 32 1E"

# Adjust food depletion rate (default 04, higher = faster drain)
# ult3edit patch edit "$ULT3" --region food-rate --data "04"
...
# ult3edit shapes view SHP0 --strings
# Then edit by offset:
# ult3edit shapes edit-string "$SHP0" --offset 0x1A3 --text "YOUR SHOP TEXT"
...
# TEXT=$(find_file "TEXT")
# ult3edit text edit "$TEXT" --record 0 --text "YOUR GAME TITLE" --backup
# ult3edit text edit "$TEXT" --record 1 --text "YOUR SUBTITLE"
